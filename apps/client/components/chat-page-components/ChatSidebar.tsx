...
import { UsersIcon, CircleIcon } from "lucide-react";
...
export default function ChatSidebar({
  ...
  return (
    <div className="hidden md:flex h-screen w-80 flex-col border-r border-border bg-card/50 backdrop-blur supports-[backdrop-filter]:bg-card/30">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {users.length > 0 ? (
          users.map((user, index) => (
            <div
              key={index}
              className="group relative flex items-center gap-3 rounded-xl p-3 transition-all duration-200 hover:bg-muted/50 cursor-pointer"
            >
